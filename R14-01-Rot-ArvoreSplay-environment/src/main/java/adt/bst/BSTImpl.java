...
public class BSTImpl<T extends Comparable<T>> implements BST<T> {
	...
	protected BSTNode<T> insert(BSTNode<T> parent, BSTNode<T> node, T element) {
		...
		if (node.isEmpty()) {
			...
			saida = this.insert(node, (BSTNode<T>) node.getLeft(), element);
		} else if (node.getData().compareTo(element) < 0) {
			saida = this.insert(node, (BSTNode<T>) node.getRight(), element);
		}
		return saida;
	}
	...
	protected BSTNode<T> remove(BSTNode<T> node) {
		...
		if (node.isEmpty() || node.isLeaf()) {
			...
			return removeNode1Grau(node);
		} else {
			return removeNode2Graus(node);
		}

	}

	private BSTNode<T> removeNode1Grau(BSTNode<T> node) {
		BSTNode<T> filhoEsquerdo = (BSTNode<T>) node.getLeft();
        BSTNode<T> filhoDireito = (BSTNode<T>) node.getRight();
        BSTNode<T> filhoNaoVazio = (filhoEsquerdo.isEmpty()) ? filhoDireito : filhoEsquerdo;
        if (node.getParent() != null) {
            if (node.getParent() != null && !node.getParent().isEmpty()
                ...
                node.getParent().setLeft(filhoNaoVazio);
            } else { 
                node.getParent().setRight(filhoNaoVazio);
            }
            filhoEsquerdo.setParent(node.getParent());
            filhoDireito.setParent(node.getParent());
        } else { 
            filhoDireito.setParent(null);
            filhoEsquerdo.setParent(null);
            this.root = filhoNaoVazio;
        }

        return filhoNaoVazio;
	}

	private BSTNode<T> removeNode2Graus(BSTNode<T> node) {
