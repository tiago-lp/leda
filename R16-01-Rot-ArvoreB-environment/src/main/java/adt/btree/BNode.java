...
public class BNode<T extends Comparable<T>> {
	...
	protected void split() {
		...
		BNode<T> left = this.copyL(media);
		BNode<T> right = this.copyR(media);

		if (parent == null) {
			parent = new BNode<T>(this.getOrder());
		...
		if (parent.size() >= this.getOrder()) {
			parent.split();
		}
	}

	private BNode<T> copyR(int media) {
		BNode<T> node = new BNode<T>(this.getOrder());
		for (int i = media + 1; i < elements.size(); i++) {
			node.addElement(this.elements.get(i));
		}
		for (int i = media + 1; i < this.children.size(); i++) {
			node.addChild(i - media - 1, this.children.get(i));
		}
		return node;
	}
